...
HWND hwMain;
const char kWindowTitle[] = "RedVSBlue_クライアント_赤";
...
DWORD WINAPI Threadfunc(void* px) {
	...
	while (1) {
		...
		send(sConnect, (const char*)&isShotA, sizeof(int), 0);
		// データ受信
		...
		int nRcvShotB = recv(sConnect, (char*)&isShotB, sizeof(Scene), 0);
