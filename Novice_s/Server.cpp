...
int WINAPI WinMain(HINSTANCE, HINSTANCE, LPSTR, int) {
	...
	while (Novice::ProcessMessage() == 0) {
		...
		if (distanceA <= a.radius + bulletB.radius) {
			...
			if (scene != Scene::S_Result)colorA = RED;
		}
		///
		/// ↑更新処理ここまで
		///
...
DWORD WINAPI Threadfunc(void* px) {
	...
	while (1) {
		...
		int nRcvShotA = recv(sConnect, (char*)&isShotA, sizeof(Scene), 0);
		...
		send(sConnect, (const char*)&isShotB, sizeof(int), 0);
	}
